...
const API_URL = 'http://localhost:3008/api/venues';
...
export const addVenue = async (venueData) => {
  try {
    ...
    if (venueData.pricingType === 'hourly' || venueData.pricingType === 'both') {
    ...
    return {
      success: false,
      error: error.message || 'An unexpected error occurred while adding the venue'
    };
  }
};

/**
 * Get venues for the currently logged in user
 * @returns {Promise<Object>} - Response with success status and data or error
 */
export const getMyVenues = async () => {
  try {
    ...
    console.log('Fetching venues from endpoint:', API_URL);
    ...
    if (!response.ok) {
      console.error('Get venues API error:', data);
      return {
        success: false,
        error: data.error || data.message || 'Failed to get venues'
      };
    }
    ...
    return {
      success: false,
      error: error.message || 'An unexpected error occurred while fetching venues'
    };
  }
};
...
export const getAvailableVenues = async () => {
  try {
    ...
    if (data && data.venues && Array.isArray(data.venues)) {
      console.log('VENUESERVICE - Raw API venues data:', JSON.stringify(data.venues, null, 2));
      
      // Make sure ownerName is preserved
      data.venues = data.venues.map(venue => {
        console.log(`VENUESERVICE - Processing venue ${venue.id}, owner name:`, venue.ownerName);
        
        // Enforce owner name if it exists in the data
        if (venue.ownerName) {
          console.log(`VENUESERVICE - Preserving owner name: ${venue.ownerName}`);
          return {
            ...venue,
      ...
      console.log('VENUESERVICE - Processed venues data:', JSON.stringify(data.venues, null, 2));
    }
    ...
    return {
      success: false,
      error: error.message || 'An unexpected error occurred while fetching available venues'
    };
  }
};
...
export const getVenueOwnerById = async (ownerId) => {
  try {
    ...
    return {
      success: false,
      error: error.message || 'An unexpected error occurred while fetching venue owner details'
    };
  }
};
...
export const reserveVenue = async (venueId) => {
  try {
    ...
    return {
      success: false,
      error: error.message || 'An unexpected error occurred while reserving the venue'
    };
  }
};
