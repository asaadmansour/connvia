  ...
  useEffect(() => {
    const fetchOwnerDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3008/api/venues/owner/${ownerId}`, {
          method: 'GET',
          headers: {
          ...
          <div className={styles.errorContainer}>
            ...
            <button 
              ...
              onClick={() => {
                ...
                fetch(`http://localhost:3008/api/venues/owner/${ownerId}`, {
                  method: 'GET',
                  headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}` // Ensure Bearer format with correct token
            ...
            <div className={styles.ownerHeader}>
              {owner.logo && (
                <div className={styles.ownerLogoContainer}>
                  <img 
                    src={`http://localhost:3008/uploads/venues/${owner.logo}`} 
                    alt={`${owner.name} logo`} 
              ...
              <div className={styles.ownerInfo}>
                ...
                <p className={styles.ownerTaxNumber}>
                  <span className={styles.label}>Tax Number:</span> {owner.tax_number}
                </p>
              </div>
            </div>
            ...
            <div className={styles.ownerVenuesSection}>
              ...
              {owner.venues && owner.venues.length > 0 ? (
                <div className={styles.venueGrid}>
                  {owner.venues.filter(venue => venue.is_available).map(venue => (
                    <div key={venue.id} className={styles.venueCard}>
                      <div className={styles.venueImageContainer}>
                        <img 
                          ...
                          onError={(e) => {
                            e.target.src = "https://via.placeholder.com/200x150?text=Venue";
                          }}
                        />
                      </div>
                      <div className={styles.venueCardDetails}>
                        <h4 className={styles.venueCardTitle}>{venue.name}</h4>
                        <p className={styles.venueCardCategory}>{venue.category || 'Venue'}</p>
                          ...
                          <span className={styles.venuePrice}>
                            {venue.pricing_type === 'hourly' 
                              ...
                              : venue.pricing_type === 'daily' 
                                ...
                                : `${venue.cost_hourly || 0} EGP/hr | ${venue.cost_daily || 0} EGP/day`}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <p className={styles.noVenuesMessage}>No available venues found for this owner.</p>
              )}
            </div>
          </div>
        ) : (
